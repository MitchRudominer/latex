...
\uncover<2->{&=  f\bigl((v_1, v_2,v_3)^{\text{T}} + (w_1, w_2,w_3)^{\text{T}} \bigr) \\}
\uncover<3->{&= f(v_1 + w_1, v_2 + w_2, v_3+w_3) \\}
\uncover<4->{&= \left(v_1 + w_1, (v_2 + w_2) + 2 (v_3 + w_3)\right)^{\text{T}}  \\}
\uncover<5->{&= \left(v_1 + w_1, (v_2 + 2v_3) +(w_2 + 2 w_3)  \right)^{\text{T}} \\}
\uncover<6->{&= \left(v_1, v_2 + 2v_3 \right)^{\text{T}} + \left(w_1, w_2 + 2 w_3 \right)^{\text{T}} \\}
\uncover<7->{&= f(v_1, v_2,v_3) + f(w_1, w_2,w_3) \\}
\uncover<8->{&= f(\bv) + f(\bw)  \\}
\end{align*}
\uncover<10->{So $f$ \emph{does} preserve vector addtion.}
\end{frame}
...
\item We will show to make a linear function out of $\bv$.
\item Let $f_{\bv} : \R^n \map \R^1$ be defined by
$$f_{\bv}(\bw) = \bv \cdot \bw$$
\item Then $f_{\bv}$ is a linear function. We will prove this in the next lesson.
...
\begin{itemize}
\item Let $\bv = (2, -1)^{\text{T}}$
\item So $f_{\bv}:\R^2 \map \R$.
\item Let $\bw = (4, 5)^{\text{T}} \in \R^2$.
