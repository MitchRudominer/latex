...
For example $\gcd(6,10) = 2$. We will eventually get back to this simple
understanding of $\gcd$, but it will pay to start with a more abstract and
sophisticated definition of $\gcd$ based on the concept of an ideal.
...
where the $a_1,a_2,\cdots, a_n$ are objects other than integers.
...
and scalar multiplication we can see that $b\in I$. (Formally we could think
of this as a proof by induction on $n$.)
\end{proof}
...
\begin{lemma}
Let $a$ be an integer. Then $(a)$ consists of all integer multiples of $a$.
For this reason another name for $(a)$ is $a\Z$.
\end{lemma}
\begin{proof}
Let $a\Z$ be the set of integer multiples of $a$. Since $(a)$ is closed under
scalar multiplication and $a\in (a)$, $a\Z\subseteq (a)$. On the other hand,
it is easy to see that $a\Z$ is closed under addition and scalar multiplication,
so $a\Z$ is an ideal containing $a$. Since $(a)$ is the smallest ideal containing
$a$, $(a)\subseteq a\Z$.
\end{proof}

\begin{in_class_example}
If $a=0$ then $(0)=\singleton{0}$. If $a=1$ then $(1) = \Z$.
If $a=2$ then $(2)$ is the set of all even numbers.
\end{in_class_example}
...
ideal containing $a$ and $b$.
\end{definition}

\begin{homework}
Show that $(a,b) = (a) + (b)$ = the set of all integer linear combinations
of $a$ and $b$.
\end{homework}
...
$(4) + (6) = (2)$ = the set of all even numbers.
\end{in_class_example}
...
\end{theorem}
\begin{proof}
Let $I$ be an ideal of $\Z$. If $I=\singleton{0}$ then $I=(0)$. Otherwise
...
such that $d\Z = a\Z + b\Z$.
\end{definition}

\begin{example}
In an in-class example above we saw that $3\Z + 5\Z = 1\Z = \Z$.
So $\gcd(3,5) = 1$.

In an in-class example above we saw that $4\Z + 6\Z = 2\Z$.
So $\gcd(4,6) = 2$.
\end{example}
...
integer $d$ such that $d$ is a common divisor of
$a$ and $b$ and if $c$ is any common divisor of $a$ and $b$ then $c\divides d$.
\end{lemma}
\begin{proof}
To say that $e$ is a common divisor of $a$ and $b$ is the same as to say that
$a\in e\Z$ and $b\in e\Z$ which is the same as to say that

\begin{equation}
\label{gcd-equation-1}
a\Z + b\Z \subseteq e\Z.
\end{equation}

To say that every common divisor of $a$ and $b$ also divides $e$ is the same
as to say that $a\Z + b\Z \subseteq c\Z \implies e \in c\Z$ which is to say

\begin{equation}
\label{gcd-equation-2}
a\Z + b\Z \subseteq c\Z \implies e\Z \subseteq c\Z.
\end{equation}
...
Then $d\Z= a\Z+b\Z \subseteq e\Z$ by (\ref{gcd-equation-1}) and,
since $a\Z+b\Z\subseteq d\Z$,
$e\Z \subseteq d\Z$ by (\ref{gcd-equation-2}).
So $d\Z = e\Z$ and since $d$ and $e$ are both non-negative by
Theorem \ref{ZIsAPID}, $e=d$.
\end{proof}
