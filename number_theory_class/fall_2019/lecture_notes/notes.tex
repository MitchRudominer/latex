...
$$A\times B = \setof{(a,b)}_{a\in A \AND b\inB}.$$
\end{definition}

\begin{example}
If $A=\Z\mod 4$ and $B=\Z\mod 9$ then $A\times B$ contains, for example, the following:
$(0,0), (1,0), (2,0), (3, 0), (0, 1), (1,1), (2,1), \cdots (0,9), (1,8), (2,8), (3,8)$.
\end{example}
...
\begin{example}
$\Z/(4) \times \Z/(9)$ is a ring. In this ring we have, for example, the following
addition and multiplication: $(2,3) \dotplus (5,4) = (3, 7)$.
$(2,3) \dottimes (5,4) = (2, 3)$.
\end{example}

A major theme we are now going to turn to in this course is the structure of
the rings $\Z/(n)$ and of the corresponding groups of units $(\Z/(n))^*$.

We will analyze the $\Z/(n)$ by dividing $n$ into three types:
\begin{enumerate}
\item[type 1.] First we analyze $\Z/(p)$ for $p$ a prime.
\item[typ 2.] Then we analyze $\Z/(p^k)$ for $p$ a prime.
\item[type 3.] Finally we look at $\Z/(n)$ for $n$ an arbitrary integer.
\end{enumerate}
...
For example, $36=2^4\cdot 3^2$. So $n=36$ is of type 3. It is of type
$n=p^2q^2$, where $p,q$ are primes.

The Chinese Remainder Theorem will tell us that

$\Z/(36) \isomorphic \Z/(4) \times \Z/(9)$.
...
the map $\pi:\Z/(36) \map \Z/(4)\times\Z/(9)$ given by $\pi(x) = \left(x\bmod 4, x\bmod 9 \right)$.
...
$(3,5)$ is in the range of $\pi$, the example above tells us that $\pi(23) = (3,5)$.

\bigskip

The examples we have been looking at above involve two simultaneous linear congruencies.
For the version of the Chinese Remainder Theorem we wish to state we will allow any finite
number of simultaneous linear congruences.
...
than 1. Let $a_1,a_2,\cdots,a_K$ be any integers. Then there is a solution to the
...
Furthermore the solution is unique module $n_1 n_2 \cdots n_K$.
...
than 1.Then there is a solution to the
system of simultaneous linear congruences
...
$$r n_1 + s m = 1.$$
Let $x=sm$. Then $n_2\divides x, n_3\divides x, \cdots x_K \divides x$ and 
$x\equiv 1 \pmod{n_1}$. So $x$ solves the system of equations.
\end{proof}
