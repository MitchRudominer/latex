...
\begin{itemize}
  \item $x\equiv y \pmod m$ iff $cx \equiv cy \pmod {cm$}.
  \item Here $c\not=0$.
  \item Notice that multiplying $x$ and $y$ be $c$ makes the statement weaker
  but multiplying $m$ by $c$ makes the statement stronger. Doing both makes the
  statement equivalent.
  \item Proof: If $cm \divides c(x-y)$ iff $m\divides (x-y)$.
  \item Example. The following two things are equivalent:
  \item $30 \equiv 80 \pmod {25}$.
  \item $6\equiv 16 \pmod 5$.
\end{itemize}
\end{frame}
