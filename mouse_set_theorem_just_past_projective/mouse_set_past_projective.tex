...
\begin{lemma}
\label{CorollaryOfWoodinGivesSemiProper}
Let $\delta$ be a limit of strong inaccessibles. Let $\Sigma$ be a collection of dense sets $D\subset \Q_{<\delta}$.
Suppose that for each $D\in\Sigma$
there are cofinally many inaccessible $\kappa<\delta$ such
that $D\intersect V_{\kappa}$ is dense in $\Q_{<\kappa}$ and semi-proper. Let $\mu > \delta$ be inaccessible.
Let $x\prec V_{\mu}$ be countable. Let $\alpha < \delta$. Then there is an inaccessible $\kappa$ with $\alpha < \kappa < \delta$
and there is a countable $y\prec V_{\mu}$ such that $x\subset y$, $y$ end-extends $x\intersect V_{\kappa}$, and for all
$D\in x\intersect \Sigma$, $y$ captures $D$.
\end{lemma}
\begin{proof}
Let $\sequence{D_i}{i=1,2,\cdots}$ enumerate $x\intersect \Sigma$. We will define an increasing sequence $x_0 \subset x_1 \subset \cdots$
and let $y=\Union{i}x_i$. 

Let $x_0=x$. Let $\kappa_1$ be inaccessible with $\alpha < \kappa_1 < \delta$ be such that
$D_1\intersect V_{\kappa_1}$ is dense in $\Q_{<\kappa_1}$ and semi-proper. Apply Lemma \ref{SemiProperEquivalence} to get a countable $x_1\prec V_{\mu}$
so that $x_0\subset x_1$, $x_1$ end-extends $x_0\intersect V_{\kappa_1}$, and $x_1$ captures $D_1\intersect V_{\kappa_1}$.
In particular, $x_1$ captures $D_1$.

Let $\kappa_2 > \kappa_1$ be inaccessible such that $D_2\intersect \kappa_2$ is dense in $\Q_{<\kappa_2}$ and semi-proper.
Apply Lemma \ref{SemiProperEquivalence} again to get a countable $x_2\prec V_{\mu}$
so that $x_1\subset x_2$, $x_2$ end-extends $x_1\intersect V_{\kappa_2}$, and $x_2$ captures $D_2\intersect V_{\kappa_2}$.
In particular $x_2$ captures $D_2$.
Because $x_2$ end-extends $x_1\intersect V_{\kappa_2}$, $x_2$ also captures $D_1$.

Continuing in this we we define an increasing sequence of inaccessible cardinals $\alpha < \kappa_1 < \kappa_2 < \cdots < \delta$ and an increasing sequence of countable
elementary submodels of $V_{\mu}$ $x = x_0 \subset x_1 \subset x_2 \subset \cdots$
such that $x_{i+1}$ end-extends $x_i \intersect V_{\kappa_{i+1}}$ and $x_i$ captures $D_i\intersect V_{\kappa_i}$.

Let $y=\Union{i}x_i$.  Then $y$ is countable, $y\prec V_{\mu}$, $x\subset y$. For all $i$, $y$ end-extends $x_i \intersect V_{\kappa_{i+1}}$,
so $y$ captures $D_i$. Letting $\kappa = \kappa_1$, we have that $y$ end-extends $x\intersect V_{\kappa}$.
\end{proof}

The following lemma is identical to the previous one except for the last sentence. In the following lemma $y$ captures every $D$ in $y\intersect\Sigma$ whereas
in the previous lemma $y$ captures every $D$ in $x\intersect\Sigma$.
...
Let $x\prec V_{\mu}$ be countable. Let $\alpha < \delta$. Then there is an inaccessible $\kappa$ with $\alpha < \kappa < \delta$
and there is a countable $y\prec V_{\mu}$ such that $x\subset y$, $y$ end-extends $x\intersect V_{\kappa}$, and for all
$D\in y\intersect \Sigma$, $y$ captures $D$.
\end{lemma}
...
Let $\delta$ be the least inaccessible greater than $\delta_n$. Since $\delta_{n+1}$ is a limit
of inaccessibles in $M$, we have $\delta_n < \lambda < \delta < \delta_{n+1}$. We will find a countable
$x \prec V_{\delta}$ so that $f\in x$,
$x\intersect\union a \in a$ and $\forall D\in x\intersect J^M_{\gamma_n}$ such that $D$ is a dense subset of $\Q_{<\delta_n}^M$, $x$ captures $D$.
Then $x\intersect V_{\lambda} \in b$ and $x\intersect V_{\lambda}$ is closed under $f$.

We will construct $x$ as an increasing union $x = \Union{k} x_k$.

Let $x_0 \prec V^M_{\delta}$ be countable, with $f\in x_0$ and such that $x_0\intersect \union a \in a$.
Let $\sequence{D_0^j}{j\in\omega} \in M$ enumerate the dense subsets of $\Q_{<\delta_n}^M$ in $x_0\intersect J^M_{\gamma_n}$.

We will define $x_1$

We will construct $x_1$ as an increasing union $x_1 = \Union{j} x_1^j$.
Let $x_1^0 = x_0$.
Suppose we have defined $x_1^0\cdots x_1^j$ countable elementary submodels of $V_{\delta}$ and we will define $x_1^{j+1}$.
Assume that for $0\leq t < j$, $x_1^t \subset x_1^{t+1}$, that $x_1^{t+1}$ captures $D_0^t$ at $e_0^t\in D_0^t$,
that $\kappa_0^t < \delta_n$ is an inaccessible such that $e_0^t\in V_{\kappa_0^t}$ and $a\in V_{\kappa_0^t}$,
and that ...




such that $x_1^0 = x_0$, $x_1^j \subset x_1^{j+1}$, $x_1^{j+1}$ captures $D_0^j$ at $e_j$

such that $x_1^0 = x_0$, $x_1^j \subset x_1^{j+1}$, $x_1^{j+1}$ end-extends $x_1^j\intersect V_{\kappa}$ for a suffiently
big $\kappa < \delta_n$, and $x_1^{j+1}$ captures $D_t$ for $t=0\cdots j$.

Let $x_1^0 = x_0$. Suppose we have defined $x_1^0\cdots x_1^j$ so that for $0\leq t <j$ we have that
$x_1^t \subset x_1^{t+1}$, $x_1^{t+1}$ end-extends $x_1^t\intersect V_{\kappa}$, and $x_1^{t+1}$ captures 
$D_s$ for $s=0\cdots t$. We will define $x_1^{j+1}$.

Temporarily working in $J^M_{\gamma_n}$, since $\delta_n$
...
Fix such a $\kappa_0^0$ with $a\in V_{\kappa_0^0}$.

As the definition of semi-proper is local, it remains true in the full $M$ that
$\kappa$ is inaccessible and that $D^0_0\intersect V_{\kappa}$ is dense and semi-proper in $\Q_{<\kappa}$.
Now working in the full $M$ we apply Lemma \ref{SemiProperEquivalence} to conclue that there is an 
$x_1^1 \prec V_{\delta}$
s.t. $x_1^1$ is countable, $x_1^0\subseteq x_1^1, x_1^1 \text{ end-extends } x_1^0 \intersect V_{\kappa}$ and $ x_1^1 \text{ captures } D_0^0$.
Let $e_0^0\in D_0^0$ be such that $x_1^1 \text{ captures } D_0^0$ at $e_0^0$.


$a, e_0,\cdots e_{j-1}\in V_{\kappa}$. Then there is a countable
$x_{j+1} \prec V_{\delta}$
...
of countable elementary submodels of $V_{\delta}$,
...
Suppose we have $x_j\prec V_{\delta}$ countable, closed under $f$, such that
...
$x \prec V_{\delta}$, $x$ is countable,
