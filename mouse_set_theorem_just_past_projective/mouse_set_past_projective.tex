...
\begin{lemma}
Let $M$ be a countable transitive model of $\ZFC$ that is projectively correct.
...
\end{lemma}
\begin{proof}
If $x\in A$ and $y\in B$ and $\varphi^M_A(x) = \varphi^M_B(x)$, then
...
functions $\sigma^M_{\varphi}$ where $\varphi$ is a projective norm on a projective set.
...
Let $h$ be a $\PiOneOmega$ code for a $\PiOneOmega$ set $A\subset\R^2$.
Suppose $x\in\R\intersect M$. Consider the the $\SigmaOneOmegaPlusOne$ statement
``$\exists y A(x,y)$''. This statement is true iff $T^h(x)$ is illfounded.
Similarly $M\models \exists y A(x,y)$ iff $(T^h(x))^M$ is illfounded.
Now if $(T^h(x))^M$ is illfounded
then $T^h(x)$ is illfounded and $\exists y A(x,y)$. But if $M$ is not
$\SigmaOneOmegaPlusOne$-correct then it may happen that $(T^h(x))^M$ is wellfounded
and so $M\models \neg\exists y A(x,y)$ but yet $T^h(x)$ is illfounded and $\exists y A(x,y)$.
