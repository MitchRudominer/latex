...
\begin{definition}
Let $M$ be a countable, transitive, projectively correct model of $\ZFC$.
Suppose $T\in M$ and $M\models ``T$ is a wellfounded $\bPi^1_{\omega}$ tree.''
We say that $T$ is \emph{correctly wellfounded} iff $T=(T^h(x))^M$ for some
$\PiOneOmega$ code $h$ and real $x\in M$, and $T^h(x)$ is wellfounded. (In this
case we say $T$ is correctly wellfounded with respect to $h$ and $x$.)
We say that $T$ is \emph{incorrectly wellfounded} iff $T=(T^h(x))^M$ for some
$\PiOneOmega$ code $h$ and real $x\in M$, and $T^h(x)$ is illfounded. (In this
case we say $T$ is incorrectly wellfounded with respect to $h$ and $x$.)
\end{definition}

The key to our proof that $\Mladder$ is quasi-$\SigmaOneOmegaPlusOne$-correct is:
...
Suppose $M\models ``T_1$ and $T_2$ are $\Pi^1_{\omega}$ trees'' and
...
Let $z$ be a real and $M$ a countable, iterable, ladder-mouse over $z$.
Let $\theta$ be the minimum of the ranks of all incorrectly wellfounded
$\bPi^1_{\omega}$ trees in $M$. Let $x\in\R\intersect M$ and let $h$ be a
$\PiOneOmega$ code. Then
$$(\exists y) A^h(x,y) \Iff M\models \rank(T^h(x)) \geq \theta.$$
\end{proof}
