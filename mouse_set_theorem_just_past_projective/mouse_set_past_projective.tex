...
F or the reader's convenience we copy here some of the definitions and results from 
...
Let $\delta>\kappa$ be a limit ordinal.
Then for any countable $x\prec V_{\delta+1}$ such that $D\in x$, $\exists y \prec V_{\delta+1}$
...
Let $x\prec V_{\delta+1}$ be countable with $D\in x$. Since $D$ is cofinal in $V_{\kappa}$, $\kappa\in x$.
Since $D$ is semi-proper,
there is an $f:[V_{\kappa+1}]^{<\omega}\map V_{\kappa+1}$ such that for all countable $z\subset V_{\kappa+1}$,
if $z$ is closed under $f$ then $z\in\semiproper(D)$. Since $x\prec V_{\delta+1}$, there is such an $f\in x$. Fix such an $f$.
...
Then $y$ is countable,  $y \prec V_{\delta+1}$ and  $x\subset y$. Furthermore, since 
