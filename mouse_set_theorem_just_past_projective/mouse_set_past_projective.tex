...
\item $\Mladder$ is projectively correct and so $\PiOneOmega$ correct.
\item Let $\sequence{\delta_n,\gamma_n}{n\in\omega}$ be a cofinal ladder over $\Mladder$.
...
\item By the previous two items, we can characterize $\Mladder$ as the least mouse
...
We are assuming $\AD(\LofR)$ in this paper but $\Det(\JofR{3})$ is sufficient for
this theorem.
...
$\Mladder$ is $\PiOneOmega$-correct but not $\SigmaOneOmegaPlusOne$-correct.
...
or not $\Sigma^1_3$ statements are true. Let $A\subset\R^2$ be $\Sigma^1_3$ and
let $x\in M_1$ and suppose there is a $y\in\R$ such that $A(x,y)$. Then
it may not be true that $M_1\models(\exists y) A(x,y)$. But
$M_1\models 1\force{\Coll(\omega,\delta)}$ ``$(\exists y) A(x,y)$'',
where $\delta$ is the Woodin of $M_1$.

Now let $a\subset\omega$ be $\SigmaOneOmegaPlusOne$. Because $\Mladder$ can tell
when $\SigmaOneOmegaPlusOne$ statements are true, $a\in\Mladder$. Thus the
heart of the proof of Theorem \ref{DefinableRealsAreInM} will be to show that
$\Mladder$ can tell when $\SigmaOneOmegaPlusOne$ statements are true.


We now develop the ideas that will allow us to prove this. We
...
In this section Projective Determinacy is sufficient.
...
Furthermore, $G^n_e$ is the projection of the tree on $\omega \times \bdelta^1_{2n+1}$ whose branches are
...
$x\in\R$ extending $s$ such that for all $i<n$, $x\in G^i_{h(i)}$ and $\varphi^{i_0}_{h(i_0),i_1} = u(i)$.
