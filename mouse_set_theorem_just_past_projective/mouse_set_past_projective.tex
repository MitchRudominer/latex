...
For $z\in\R$, let  $\theta_z<\omega_2^{\Mladder(z)}$ be the
...
\begin{definition}
Fix $z\in\R$ and let $G$ be given by Lemma \ref{LadderGeneric} for $\Mladder(z)$. That is,
let $\gamma=\ord(\Mladder(z))$ and let $G\subset \Q_{<\gamma}^{\Mladder(z)}$ be such that for
$n\in\omega$, $G\intersect J^{\Mladder(z)}_{\delta_n}$ is
$\Q_{<\delta_n}^{\Mladder(z)}$-generic over $J^{\Mladder(z)}_{\gamma_n}$.
Let $\pi:\Mladder(z)\to\Mstar$ be the stationary tower ultrapower.
We will capture this setting by saying that  $\pi:\Mladder(z)\to\Mstar=\Mladder(z)[G]$
is a \emph{ladder stationary tower embedding.}

Let $g$ be $\Col(\omega,\delta)$-generic over $\Mladder(z)$ for some
$\delta\in\Mladder(z)$.
$\Mladder(z)[g]$ can be re-organized as $\Mladder(\zprime)$ for some $\zprime\in\R$.
Let $\psi:\Mladder(\zprime)\to\Nstar=\Mladder(\zprime)[H]$ be the ladder Stationary Tower embedding.
We will capture this setting by saying that
$\psi:\Mladder(z)[g]\to\Nstar=\Mladder(\zprime)[g][H]$ is a ladder stationary tower embedding \emph{above $g$}.
\end{definition}
...
Let $\pi:\Mladder\to\Mstar=\Mladder[G]$ be a ladder stationary tower embedding.
...
$S=(T^{h_2}(y))^{\Mstar}$ and $x,y\in\Mladder[g]$
where $g$ is $\Col(\omega,\delta)$ generic over $\Mladder$ for some $\delta\in\Mladder$.
\end{proof}
...
Then $(T^h)^{M[g_1]}=(T^h)^{M[g_2]}$.

Furthermore $\sigma^{M[g_1]} = \sigma^{M[g_2]}$ where
$\sigma^{M[g_i]}:(T^h)^{M[g_i]}\map(T^h)^V$ is the tree embedding from
...
\begin{definition}
\label{T_at_delta}
Let $M$ be a countable transitive model of $\ZFC$ - Replacement + ``There are cofinally many inaccessible cardinals.'' Suppose that every set-generic extension of $M$ is projectively correct.
...
We also define $\sigma^{M,\delta}:(T^h(x))^{M,\delta} \map (T^h(x))^V$ by
$\sigma^{M,\delta}=\sigma^{M[g]}$ from Definition \ref{norm_embedding_def},
where $g$ is any
$\Col(\omega,\delta)$-generic over $M$.

The definitions makes sense because by Lemma \ref{applyhjorthcorollary}, relatized to $x$
...
We define $\sigma^{M,\delta_1,\delta_2} = (\sigma^{M,\delta_2})^{-1}\circ\sigma^{M,\delta_1}$
\end{definition}
