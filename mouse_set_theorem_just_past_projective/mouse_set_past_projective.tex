...
all generic extensions of a sufficiently correct, countable, transitive model of Set Theory
...
\begin{lemma}[Hjorth]
\label{hjorthlemma}
Let $M$ be a countable, transitive model of $\ZFC$ and  $\delta>\omega$ a cardinal of $M$. Let $n>1$ be an integer.
Suppose that whenever $\P\in V^M_{\delta+1}$ is a poset and $g$ is $\P$-generic over $M$, then
$M[g]$ is $\Sigma^1_n$-correct. Let $g_1$ and $g_2$ be $\Col(\omega, \delta)$-generic over $M$.
Let $E\subset\R^2$ be a $\Sigma^1_n$, thin equivalence relation.
...
we may now apply Hjorth's Lemma \ref{hjorthlemma} to conclude that if $g$ is any $\Col(\omega,\delta_{2n+2})$-generic over $N$,
...
$\neg E(\tau^{M[h_1]},\tau^{M[h_2]})$.
\end{claim}
\begin{subproof}[proof of Claim 1]
Let $\tau\in M$ be such that $x=\tau^{M[g_1]}$. By hypothesis $M[g_1\times g_2]$ is $\Sigma^1_n$-correct
so $M[g_1\times g_2]\models \neg E(\tau^{M[g_1]},\tau^{M[g_2]})$.
$$M\models 1\longforce{\Col(\omega, \delta)\times \Col(\omega, \delta)} \neg E(\dot{\tau}_{\text{left}},\dot{\tau}_{\text{right}}).$$
\end{subproof}

To obtain a contradiction and prove the lemma we will show

\begin{claim}[Claim 2]
There is a set $H$ such that for all $h_1,h_2\in H$ wit $h_1\not= h_2$,
$h_1\times h_2$ is $\Col(\omega, \delta)\times \Col(\omega, \delta)$-generic over $M$
and $\setof{\tau^{M[h]}}{h\in H}$ is a perfect set of reals.
\end{claim}
