...
is the \emph{ladder Stationary Tower embedding.}
...
$\psi:\Mladder(z)[g]\to\Nstar=\Mladder(\zprime)[g][H]$ is the ladder Stationary Tower embedding \emph{above $g$}.
\end{definition}

Now we turn to a discussion of the proof of Lemma \ref{Nsat_trees_are_cofinal}.
Let $T\in\Mladder$ be a $\bPi^1_{\omega}$-tree that is correctly wellfounded for $\Mladder$.
Let $\pi:\Mladder\to\Mstar=\Ult(M,G)$ be the ladder Stationary Tower embedding.
...
Let $\pi:\Mladder\to\Mstar=\Mladder[G]$ be the ladder Stationary Tower embedding.
Let $T,S\in\wfp(\Mstar)$ be $\bPi^1_{\omega}$-trees in $\Mstar$.
Suppose that $T$ is correctly wellfounded for $\Mstar$ and
$S$ is incorrectly wellfounded for $\Mstar$. Suppose further that $T$ is $\PiOneOmega(x)$ for
$x\in\R\intersect\Mladder$, and $S$ is $\PiOneOmega(y)$ for $y\in\R\intersect\Mladder[g]$,
...
$S=(T^{h_2}(y))^{\Mstar}$ and $x\in\Mladder$ and $y\in\Mladder[g]$
where $g$ is $\Col(\omega,\delta)$ generic over $\Mladder$ for some $\delta\in\Mladder$.
\end{proof}
