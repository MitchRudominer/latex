...
fix $\PiOneOmega$ codes $h_1, h_2$ and $x,y\in\R\intersect \Mladder[g]$
...
Let $x\in\R\intersect M$ and let $\varphi$ be a $\Sigma^1_n(x)$ norm for some $n$.
...
Let $h$ be a $\PiOneOmega$ code. Then $(T^h(x))^{M[g_1]}=(T^h(x))^{M[g_2]}$
and $\sigma^{M[g_1],h} = \sigma^{M[g_2],h}$ where
$$\sigma^{M[g_i],h}:(T^h)^{M[g_i]}\map(T^h)^V$$
...
Suppose $(s,u)\in (T^h)^{M[g_1]}$. Let $n=\length(s)=\length(u)$. Then there is
$y_1\in\R\intersect M[g_1]$ extending $s$ such that $(\forall i<n)\, x\in G^{i}_{h(i)}$ and
$u(i)=(\varphi^{i_0}_{h(i_0),i_1})^{M[g_1]}(y_1)$.

By Corollary \ref{hjorthcorollary} there is a $y_2\in\R\intersect M[g_2]$ such that
\begin{itemize}
\item $y_2 \restr n = y_1 \restr n$
\item $y_2\in \Intersection{i<n} G^i_{h(i)}$.
\item For $i<n$, $\varphi^{i_0}_{h(i_0),i_1}(y_2) = \varphi^{i_0}_{h(i_0),i_1}(y_1)$
\end{itemize}

So then for $i<n$, $(\varphi^{i_0}_{h(i_0),i_1})^{M[g_2]}(y_2) =
(\varphi^{i_0}_{h(i_0),i_1}(y_1))^{M[g_1]}$.
...
That $\sigma^{M[g_1],h} = \sigma^{M[g_2],h}$ now follows immediately.
\end{proof}
...
Then we define $T^h(x)^{M,\delta}=(T^h(x))^{M[g]}$ , and
$\sigma^{M,\delta}=\sigma^{M[g]}$, and
...
Let $\bar{T}^h(x)^{M,\delta}=\ran(\sigma^{M,\delta,h})$. Then $\bar{T}^h(x)^{M,\delta}$
is a subtree of $(T^h(x))^{V}.$  $\bar{T}^h(x)^{M,\delta}=\big\{(s,u)\in (T^h(x))^{V} \, \mid \,
\exists g \Col(\omega,\delta)$-generic over $M$ and $\exists y \in \R\intersect M[g]$ s.t.
$y$ extends $s$ and $(\forall i<\length(u))\, y\in G^{i}_{h(i)}$ and
$u(i)=(\varphi^{i_0}_{h(i_0),i_1})(y) \big\}$.


If $\delta_1<\delta_2$ then $\ran(\sigma^{M,\delta_1})\subseteq \ran(\sigma^{M,\delta_2})$
and $\bar{T}^h(x)^{M,\delta_1}\subseteq \bar{T}^h(x)^{M,\delta_2}$.


Let $\sigma^{M,\infty}$ be the inverse of the transitive collapse of
$$\bigcup\setof{\ran(\sigma^{M,\delta})}{\delta\in M}.$$
