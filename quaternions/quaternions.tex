...
By Lemma \ref{lemma:mult_deriv}, $\Ad(q)q = \exp(q) q  \exp(q)^{-1}$ and by
Corollary \ref{corollary:exp_commute} $q$ commutes with $\exp(q)$. It may also be
enlightening to perform that calculation explicitly:
\begin{multline*}
\begin{pmatrix}
\cos(c) \\ \sin(c)v
\end{pmatrix}
