...
\begin{definition}
The \emph{unit quaternions} refers to the set of quaternions with norm 1.
\end{definition}

\begin{corollary}
The unit quaternions is naturally a three-dimensional Lie group, isomorphic to $\SU(2)$.
\end{corollary}
\begin{proof}
...
\begin{lemma}
(a) $\H$ with the commutator bracket is the Lie algebra of the Lie group $\H-\singleton{0}$.


(b) $\H_0$ with the commutator bracket is the Lie algebra of the Lie group of unit quaternions.
\end{lemma}
\begin{proof}
...
For part (b), let $S\subset \H$ be the Lie group of unit quaternions.
Given part (a), in order to prove part (b) it suffices to show that for $q\in\H$,
$q$ is tangent to $S$ at 1 iff $q\in\H_0$.

Let $s(t)$ be a path in $S$ with $s(0) = 1$. We want to show that the real part of $q = \sprime(0)$ is zero.
...
Conversely let $q\in\H_0$. We want to show that $q$ is tangent to $S$ at 1. 
If $q=0$ then it is clearly tangent to $S$ so suppose $q\not=0$.
...
\end{pmatrix}
.$$
Then $s(0) = 1$, $s(t)\in S$ for all $t$, and
$$
\sprime(t) = 
\begin{pmatrix}
