...
$\sigma:\SU(2)\map\R\P^3$ be a covering map.
There are ways to visualize $\S^3$ and $\R\P^3$ because they are both homeomorphic
 to spaces constructed by gluing together subsets of $\R^3$. In this section we will use this
 to visualize the covering map $\pi:\SU(2)\map\SO(3)$. The following diagram commutes:
...
$$
 ...
 By Example ??? from \cite{Lee_Topology_Book},
  ...
  its boundary sphere. We call this the model space of $\R\P^3$ and below we will consider $\R\P^3$ to
  be defined as this model space. We will describe an explicit homeomorphism $\mu$ from $\SO(3)$
  to this model of $\R\P^3$. Let $A\in\SO(3)$. 
  Then $\Ad^{-1}[f^{-1}(A)] =\singleton{q, -q}$ for some $q\in\H_1$, and by swapping
  $q$ with $-q$ if necessary we may write
  $$q=\begin{pmatrix}\cos(\varphi) \\ \sin(\varphi) \vec{v}\end{pmatrix}$$
 with $0\leq\varphi\leq\pi/2$ and $|\vec{v}|=1$. Let $\theta=2\varphi$ so
 $0\leq\theta\leq\pi$ and $A$ is the matrix of the rotation by $\theta$ radians about $\vec{v}$.
 We let  $\mu(A)$ be the point $\sin(\varphi) \vec{v}$ in the model space of $\R\P^3$.

 Notice that if $\varphi=0$ then $q=1$, $A = I$, and $\mu(I)$ is the center of the ball.
 If $\varphi=\pi/2$ then $\theta=\pi$ and $A$ is the matrix of the rotation by $\pi$ radians about $\vec{v}$
 and $\mu(A)$ is the point $\vec{v}$ on the bounary of the ball. In this case since $\cos(\varphi)=0$
 our initial choice of $q$ vs $-q$ was arbitrary, so we might have instead mapped $A$ to $-\vec{v}$.
 But that is OK since we are identifying $\vec{v}$ and $\vec{-v}$ in the quotient space model of $\R\P^3$. 
 The function $\mu$ is clearly a homeomorphism.

 By Example ??? from \cite{Lee_Topology_Book},
 $\S^3$ is homeomorphic to the quotient space obtained by starting with two
 copies of $\bar{\B^3}$  (let's call them $B_{\text{left}}$ and
 $B_{\text{right}}$) and identifying corresponding points on
 their respective boundary spheres. We call this the model space of $\S^3$ and below we will consider $\S^3$ to
 be defined as this model space. We will describe an explicit homeomorphism $\tau$ from $\SU(2)$
 ...
 Now we describe the induced double cover from the quotient space model of $\S^3$ to the quotient space model of $\R\P^3$.
