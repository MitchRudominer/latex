...
\begin{definition}
Let $q\in\H_1$. Then an \emph{angle-axis} representation of $q$ is is a pair $(\theta,\vec{v})$ such that
...
\emph{is} and angle-axis representation of $q$.

\begin{remark}
For all $q\in\H_1$ other than $q=1$ and $q=-1$, $q$ has a unique angle-axis represention.
In every angle-axis representation of $q=1$, $\theta=0$ but $\vec{v}$ is arbitrary, and similarly
in every angle-axis representation of $q=-1$, $\theta=\pi$ but $\vec{v}$ is arbitrary.
\end{remark}

\begin{definition}
For $q\in\H_1, q \not= 1, q\not=-1$ let $\ang(q)$ and $\axis(q)$ be the unique
$\theta$ and $\vec{v}$ such $(\theta,\vec{v})$
is an angle-axis representation of $q$. Define $\ang(1)=0$, $\axis(1)=\vec{i}$.
Define $\ang(-1)=\pi$, $\axis(-1)=\vec{i}$.
\end{definition}
...
$$
where $\theta=\ang(q), \vec{v} = \axis(q), \vec{v} \cdot \vec{v} = 1,\vec{v}_1 \cdot \vec{v}_1 = 1, \vec{v}_1 \cdot \vec{v} = 0.$
...
Then $\Pi_q$ is rotation by $2\ang(q)$
about $\axis(q)$. For $q=1$ and $q=-1$, $\Pi_q$ is the identity.
\end{theorem}
\begin{proof}
...
that $q\not=1$, $q\not=-1$. Let $\theta=\ang(q)$ and $\vec{v} = \axis(q)$.
...
if we identify $\H_0$ with $\R^3$, then $\Ad(q)$ is rotation by $\ang(q)$ about $\axis(q)$. This
...
It is possible to \emph{visualize} $\S^3$ and $\R\P^3$ because they are both homeomorphic
 ...
 For any point $p$ in the disjoint union of $B_{\text{left}}$ and  $B_{\text{left}}$ let $[p]\in\S^3$
be its image under the quotient map. Similarly for any $s\in B$ let $[s]\in \R\P^3$ be its image
...
Fix $q\in\H_1$ and write $$q=\begin{pmatrix}\cos(\varphi) \\ \sin(\varphi) \vec{v}\end{pmatrix}$$
...
Let $\theta=2\varphi$
so $0\leq\theta\leq 2\pi$. Notice that $2(\pi-\varphi) = -\theta + 2\pi$.
Let $A=\pi(Q(q))=\pi(Q(-q)\in\SO(3)$. Recall that $A$ is the matrix
of the rotation by $\theta$ radians about the axis $v$; equivalently the rotatoin of
$-\theta$ radians about the axis $-v$; equivalently the rotation of $2\pi - \theta$ radians
about the axis $-v$.

\begin{warning}
In Section \ref{section:rotations} we were using $\theta$ for $\ang(q)$ and in this
subsection we have switched to using $\varphi$ for $\ang(q)$ and $\theta$ for $2\varphi$.
\end{warning}


We now define $\tau(Q(q))$, and $\sigma(\tau(Q(q))) = \mu(\pi(Q(q)))= \mu(\pi(Q(-q)))$,
conditioning on whether $q$ is northern, southern, or equitorial.
Note that if $q$ is northern then $-q$ is southern, and vice versa. $q$ is equitorial iff $-q$ is.
\begin{itemize}
\item $q$ is northern iff $0\leq\varphi<\frac{\pi}{2}$ and $0\leq\theta<\pi$. In this case define:
  \begin{itemize} 
  	\item $\tau(Q(q))=[p]$  where $p$ is the point $\sin(\varphi) \vec{v}$  in the interior of $B_{\text{right}}$. 
  	\item $\sigma(\tau(Q(q)))=[s]$ where $s$ is the point $\sin(\varphi) \vec{v}$ in the interior of $B$.
  \end{itemize}
\item $q$ is southern iff $\frac{\pi}{2}<\varphi\leq\pi$ and $\pi < \theta < 2\pi$. In this case define: 
  \begin{itemize}
  	\item $\tau(Q(q))=[p]$ where $p$ is the point $\sin(\varphi) \vec{v}$ in the interior of 
  	$B_{\text{left}}$. 
  	\item $\sigma(\tau(Q(q)))=[s]$ where $s$ the point $-\sin(\varphi) \vec{v}$ in the interior of $B$.
   \end{itemize}
\item $q$ is equitorial iff $\varphi=\frac{\pi}{2}$ and $\theta=\pi$. In this case define:
  \begin{itemize}
  	\item $\tau(Q(q))=[p_1]=[p_2]$  where $p_1$ is the point $\vec{v}$ in the boundary sphere of
$B_{\text{left}}$ and $p_2$ is the point  $\vec{v}$ in the boundary sphere of
$B_{\text{right}}$. 
    \item $\sigma(\tau(Q(q)))=[s_1]=[s_2]$ where $s_1$ is the point
$\vec{v}$ in the boundary sphere of $B$ and $s_2$ is the point $-\vec{v}$ in the boundary sphered of $B$
  \end{itemize}.
\end{itemize}

An informal but suggestive way to remember what is happening is the following:
\begin{itemize}
\item The interior of $B_{\text{right}}$ represents the northern hemisphere of $\S^3$
\item The origin of $B_{\text{right}}$  represents the north pole of $\S^3$.
\item The interior of $B_{\text{left}}$ represents the southern hemisphere $\S^3$.
\item The origin of $B_{\text{left}}$ represents the spouth pole of $\S^3$.
\item The identified boundary spheres of $B_{\text{left}}$ and $B_{\text{right}}$ represent the equator of $\S^3$.
\item The point $\sin(\varphi)\vec{v}$ in the interior of $B$ represents 
the rotation by $\theta=2\varphi$ radians about $v$, for $0\leq\varphi<\frac{\pi}{2}$, and $0\leq\theta<\pi$.
\item The origin of $B$ represents the identity rotation.
\item The point $\vec{v}$ on the boundary sphere of $B$ represents the rotation of $\pi$ radians about $v$,
equivalently the rotation of $\pi$ radians about $-v$.
\item The function $\sigma$ restricted to $B_{\text{right}}$ acts like the identity on $\bar{\B}$.
\item The funciton $\sigma$ restricted to $B_{\text{left}}$ acts like the antipodal map on $\bar{\B}$.
\item Given a point $s=\sin(\varphi)\vec{v}$ in the interior of $B$, it's two inverse images under the double cover are the points
$p_1=\sin(\varphi)\vec{v}\in B_{\text{right}}$ and $p_1=-\sin(\varphi)\vec{v}\in B_{\text{left}}$.
\item Given the antipodal points $s=\vec{v}, -s = -\vec{v}$ in the boundary of $B$ the four
\end{itemize}

Informally, $\sigma:B_{\text{right}}\map B$ is the identity map on $\bar{\B}$ and
$\sigma:B_{\text{left}}\map B$ is the antipodal map on $\bar{\B}$.


For any point $p$ in the disjoint union of $B_{\text{left}}$ and  $B_{\text{left}}$ let $[p]\in\S^3$
be its image under the quotient map. Similarly for any $q\in B$ let $[q]\in \R\P^3$ be its image
under the quotient map. For any $q\in B$, $[q]\in \R\P^3$ has two inverse images in $\S^3$ under $\sigma$.
If $q$ is in the interior of $B$
